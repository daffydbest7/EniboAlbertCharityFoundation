...
const createArticle = async (req, res)=>{
    ...
    try{
     ...
     const insertArticleValue = [ image, title, content, user.id ];
     ...
     return res.status(200).json({
        ...
        data:{
            ...
            image: insertArticleCheck.image
...
const updateArticleById = async (req, res)=>{
    ...
    if(!user){
     ...
     try{
        // Admin can update posts by Id
        const updateArticleQuery = 'UPDATE article SET title = $1, content = $2 WHERE id = $3 RETURNING *;' ;
        const updateValue = [newTitle , content, articleId ]; 
        const updateResult = await db.query(updateArticleQuery, updateValue);
        const checkResult = updateResult.rowCount;   
        //return the response
        if(!checkResult){
            return res.status(403).json({
                status:"error",
                error: "invalid article id.. cannot update"
            })
        }else {
            return res.status(201).json({
                status:"success",
